...
6. **Run lint and format checks** — run `npm run lint` and `npm run format:check` from the project root. If lint fails, report the errors and stop. If format issues are found, run `npm run format` to fix them automatically, re-stage with `git add -A`, then continue to commit.
